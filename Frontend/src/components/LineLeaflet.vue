        ...
        <form @submit.prevent="saveDrawingData">
          ...
          <div class="form-group">
            ...
            <select v-model="formData.type" required>
              ...
              <option value="electricity">สายไฟฟ้า</option>
              <option value="pipe">ท่อประปา</option>
...
const TYPE_ICON_URLS = {
  electricity: '/markers/route.png',
  pipe: '/markers/route.png',
  road: '/markers/route.png'
};


const TYPE_ALIASES = {
  electricity: 'electricity',
  'สายไฟฟ้า': 'electricity',
  pipe: 'pipe',
  'ท่อประปา': 'pipe',
  road: 'road',
  'ถนน': 'road'
...
function normalizeType(type) {
  if (type === null || type === undefined) return 'electricity';
  const raw = String(type).trim();
  const rawLower = raw.toLowerCase();
  return TYPE_ALIASES[raw] || TYPE_ALIASES[rawLower] || 'electricity';
}

function displayTypeName(typeKey) {
  const key = normalizeType(typeKey);
  if (key === 'electricity') return 'สายไฟฟ้า';
  if (key === 'pipe') return 'ท่อประปา';
  if (key === 'road') return 'ถนน';
  return 'สายไฟฟ้า';
}
...
async function loadExistingData() {
  try {
    const response = await axios.get('http://localhost:3000/api/linestring');
    ...
    allFeatures.value.forEach((feature, idx) => {
      ...
      if (feature.geometry.type === 'LineString') {
        const layer = L.polyline(
          ...
          getStyleByType(typeKey, feature.properties.id)
        );
        layer.bindPopup(`
          <div>
        ...
        typeLayerGroups[typeKey].addLayer(layer);
        ...
        layerMap.set(feature.properties.id, polylineArr);
...
if (map && rowData.geometry?.type && rowData.geometry?.coordinates) {
      ...
      layers.forEach(layer => {
        if (layer.getBounds) {
          if (!groupBounds) {
            ...
            groupBounds = groupBounds.extend(layer.getBounds());
          }
        }
      });
      if (groupBounds) {
...
function initMap() {
  if (map) return;
  map = L.map("map").setView([12.781805321366827, 101.29280425205154], 14);
