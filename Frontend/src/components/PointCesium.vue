...
function setupErrorHandler() {
  ...
  try {
    ...
    viewer.scene.renderError.addEventListener(function(scene, error) {
      ...
      // พยายามแก้ไขโดยปิดฟีเจอร์ที่อาจทำปัญหา
      try {
...
onMounted(async () => {
  ...
  try {
    ...
    if (!viewer) {
      ...
      // --- เพิ่ม 3D Buildings Layer ---
      try {
        const buildingsTileset = await Cesium.Cesium3DTileset.fromIonAssetId(96188);
        viewer.scene.primitives.add(buildingsTileset);
        console.log('3D Buildings layer loaded');
      } catch (err) {
        console.error('Failed to load 3D Buildings layer:', err);
      }
...
#cesiumContainer.map-expanded {
  ...
  height: auto (100vh - 60px) !important;
}
