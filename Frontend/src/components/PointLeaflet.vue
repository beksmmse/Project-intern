        ...
        <form @submit.prevent="saveDrawingData">
          ...
          <div class="form-group">
            ...
            <select v-model="formData.type" required>
              ...
              <option value="spotlight">สปอตไลท์</option>
              <option value="fire_hydrant">หัวดับเพลิง</option>
              <option value="signal_light">ไฟสัญญาณ</option>
              <option value="swan_light">เสาหงษ์</option>
              <option value="water_level">เครื่องตรวจสอบระดับน้ำ</option>
              <option value="aed">เครื่องกระตุ้นหัวใจ</option>
              <option value="pole22_5m">เสาไฟฟ้า 22.5 ม.</option>
              <option value="pole12m">เสาไฟฟ้า 12 ม.</option>
              <option value="pole8m">เสาไฟฟ้า 8 ม.</option>
              <option value="road_light">เสาไฟฟ้าแสงสว่างขาเดียว</option>
              <option value="road_sign">ป้ายซอย</option>
              <option value="bin">ถังขยะ</option>
              <option value="shrine">ศาลเจ้า</option>
...
const TYPE_ICON_URLS = {
  bridge: '/markers/bridge.jpg',
  spotlight: '/markers/spotlight.jpg',
  fire_hydrant: '/markers/fire_hydrant.jpg',
  signal_light: '/markers/signal_light.jpg',
  swan_light: '/markers/other.png',
  water_level: '/markers/water_level.jpg',
  aed: '/markers/aed.jpg',
  pole22_5m: '/markers/pole22.5m.jpg',
  pole12m: '/markers/pole12m.jpg',
  pole8m: '/markers/pole8m.jpg',
  road_light: '/markers/road_light.jpg',
  road_sign: '/markers/road_sign.jpg',
  bin: '/markers/bin.jpg',
  shrine: '/markers/other.png',
  temple: '/markers/temple.jpg',
  other: '/markers/other.png',
  store: '/markers/Store.jpg'
};
...
const TYPE_ALIASES = {
  ...
  'หัวดับเพลิง': 'fire_hydrant',
  signal_light: 'signal_light',
  'ไฟสัญญาณ': 'signal_light',
  swan_light: 'swan_light',
  'เสาหงษ์': 'swan_light',
  water_level: 'water_level',
  'เครื่องตรวจสอบระดับน้ำ': 'water_level',
  aed: 'aed',
  'เครื่องกระตุ้นหัวใจ': 'aed',
  pole22_5m: 'pole22_5m',
  'เสาไฟฟ้า 22.5 ม.': 'pole22_5m',
  pole12m: 'pole12m',
  'เสาไฟฟ้า 12 ม.': 'pole12m',
  pole8m: 'pole8m',
  'เสาไฟฟ้า 8 ม.': 'pole8m',
  road_light: 'road_light',
  'เสาไฟฟ้าแสงสว่างขาเดียว': 'road_light',
  road_sign: 'road_sign',
  'ป้ายซอย': 'road_sign',
  bin: 'bin',
  'ถังขยะ': 'bin',
...
function displayTypeName(typeKey) {
  ...
  if (key === 'bridge') return 'สะพาน';
  if (key === 'spotlight') return 'สปอตไลท์';
  if (key === 'fire_hydrant') return 'หัวดับเพลิง';
  if (key === 'signal_light') return 'ไฟสัญญาณ';
  if (key === 'swan_light') return 'เสาหงษ์';
  if (key === 'water_level') return 'เครื่องตรวจสอบระดับน้ำ';
  if (key === 'aed') return 'เครื่องกระตุ้นหัวใจ';
  if (key === 'pole22_5m') return 'เสาไฟฟ้า 22.5 ม.';
  if (key === 'pole12m') return 'เสาไฟฟ้า 12 ม.';
  if (key === 'pole8m') return 'เสาไฟฟ้า 8 ม.';
  if (key === 'road_light') return 'เสาไฟฟ้าแสงสว่างขาเดียว';
  if (key === 'road_sign') return 'ป้ายซอย';
  if (key === 'bin') return 'ถังขยะ';
  if (key === 'shrine') return 'ศาลเจ้า';
  if (key === 'temple') return 'วัด';
  if (key === 'store') return 'ร้านค้า';
  if (key === 'other') return 'อื่นๆ';
  return 'อื่นๆ';
}

// ฟังก์ชันสำหรับแสดงประเภทใน info-box
...
async function loadExistingData() {
  try {
    ...
    allFeatures.value.forEach(feature => {
      const markerType = normalizeType(feature.properties.type);
      const marker = L.marker(
        [feature.geometry.coordinates[1], feature.geometry.coordinates[0]], 
        { 
          icon: getIconByType(feature.properties.type),
          markerType: markerType, // เพิ่มข้อมูลประเภทใน marker options
          featureId: feature.properties.id // เพิ่ม ID ด้วยเพื่อง่ายต่อการค้นหา
        }
      );
...
//<p><strong>ที่อยู่:</strong> ${feature.properties.address}</p>
      ...
      layerMap.set(feature.properties.id, marker);
      ...
      typeLayerGroups[t].addLayer(marker);
    });
...
function initGroupedLayerControl() {
  ...
  baseMaps.value = {
    ...
    "Dark Mode": L.tileLayer("https://www.deemap.com/special/gwc/service/tms/1.0.0/Barge_Procession:Bankhai@EPSG%3A900913@png8/{z}/{x}/{-y}.png8", { 
      maxZoom: 20, 
...
function initMap() {
  if (map) return;
  map = L.map("map").setView([12.781805321366827, 101.29280425205154], 10);
